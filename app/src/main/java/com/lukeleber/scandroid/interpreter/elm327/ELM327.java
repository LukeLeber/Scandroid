...
/**
 * <p>An {@link killgpl.scandroid.interpreter.Interpreter} that utilizes the ELM327 integrated
        ...
        extends AbstractInterpreter<String, String, Void>
{
    ...
    /**
     * Constructs an {@link killgpl.scandroid.interpreter.elm327.ELM327} with the provided {@link
     * killgpl.scandroid.io.CommunicationInterface}
     *
     * @param com
     *         the {@link killgpl.scandroid.io.CommunicationInterface} to communicate with the
            ...
            IOException
    {
        ...
        if (request instanceof ServiceRequest)
        {
            ...
            String s = String.format("%02x%02x", sr.getService().getID(), sr.getPID().getID());
        ...
        else if (request instanceof ConfigurationRequest)
        {
            ...
            String s = String.format(cr.getOption().getOption(), cr.getArgs());
            toSend = new byte[s.length() + 1];
            System.arraycopy(s.getBytes(), 0, toSend, 0, s.length());
        }
        else
        {
            throw new UnsupportedOperationException(
                    "Request types of " + request.getClass().getSimpleName() +
                            " are not supported by the " + getName() + " interpreter"
            );
        }
        toSend[toSend.length - 1] = 0x0D;
        getCommunicationInterface().getOutputStream().write(toSend);
    }
    ...
    /**
     * The {@link killgpl.scandroid.interpreter.ResponseListener} that handles incoming responses
     * from the ELM327 board.  The ELM327 responds differently to different types of {@link
     * killgpl.scandroid.interpreter.Request Requests}: <ul> <li>{@link
     * killgpl.scandroid.interpreter.ConfigurationRequest} - An implementation defined value is sent
     * from the ELM327, generally the string 'OK' upon success.  This value is passed to the
     * provided {@link killgpl.scandroid.interpreter.Handler} verbatim</li> <li>{@link
     * killgpl.scandroid.interpreter.ServiceRequest} - The first two characters sent from the ELM327
     * is "40" plus the service ID; thus a request using service ID 1 ({@link
     * killgpl.scandroid.sae.Service#LIVE_DATASTREAM}) shall produce "41".  The following two
     * characters are the PID/TID/OBDMID/INFOTYPE ID of the request; so PID #1 ({@link
     * killgpl.scandroid.sae.detail.AppendixB#MONITOR_STATUS}) shall produce "01".  The following
     * characters sequence is the actual data response from the ELM327.  So to sum everything up, a
     * service request $02$04 would produce the following response: "4204??" where "??" is a single
     * hex-byte representing the calculated engine load.  This {@link killgpl.scandroid.interpreter.ResponseListener}
     * trims off the first four characters of each response, invoking the requested {@link
     * killgpl.scandroid.sae.PID.Unmarshaller}</li> on only the actual data. </ul>
     *
     * @param <T>
    ...
    {
        /// The {@link killgpl.scandroid.interpreter.Request Requests} that was sent
        private final Request<String, T> request;

        /**
         * Constructs a DefaultResponseListener for the provided {@link
         * killgpl.scandroid.interpreter.Request}
         *
         * @param request
         *         the {@link killgpl.scandroid.interpreter.Request} that is being sent
         */
        ...
        public void onSuccess(String resp)
        {
            ...
            if(resp.substring(0, resp.length() - 1).equals(OpCode.ELM327_NO_DATA))
            {
            ...
            else if (request instanceof ServiceRequest)
            {
                ...
                if (!resp.startsWith(String.valueOf(40 + serviceRequest.getService().getID()).toUpperCase()))
                {
                    throw new IllegalStateException("Invalid message header received" + " expected " + String.valueOf(40 + serviceRequest.getService().getID()).toUpperCase() + " but received " + resp);
                }
                if(resp.startsWith("44")) // Special case for "clear DTCs"
                {
                    handler.onResponse(serviceRequest.getUnmarshaller().invoke("DTCs Cleared".getBytes()));
                    return;
                }
                resp = resp.substring(2);
                if (!resp.startsWith(String.format("%02x", serviceRequest.getPID().getID()).toUpperCase()))
                {
                    throw new IllegalStateException("Invalid message header received: " + resp.substring(0, 2) + " but expected " + String.format("%02x", serviceRequest.getPID().getID()));
                }
                resp = resp.substring(2);
                byte[] parsed = new byte[resp.length() / 2];
                for (int i = 0; i < resp.length() - 2; i += 2)
                {
                    parsed[i / 2] = (byte) (Integer.decode("0x" + resp.substring(i, i + 2)) & 0xFF);
                }
                if (handler != null)
                {
                    handler.onResponse(serviceRequest.getUnmarshaller().invoke(parsed));
                }
            }
            else
            {
                if (handler != null)
                {
                    ((ConfigurationRequest<String, String>) request).getHandler().onResponse(resp);
                }
            }
        }
